...
function buildUnsignedRedirectUrl(baseUrl, originalUrl) {
  const normalizedBaseUrl = normalizeRedirectBaseUrl(baseUrl);
  const normalizedOriginalUrl = String(originalUrl || "").trim();

  if (!normalizedBaseUrl || !normalizedOriginalUrl) {
    throw new Error("Direct redirect requires base URL and original URL.");
  }

  const redirectUrl = new URL(DEFAULT_REDIRECT_PATH, normalizedBaseUrl);
  redirectUrl.searchParams.set("u", encodeStringAsBase64Url(normalizedOriginalUrl));
  return redirectUrl.toString();
}
